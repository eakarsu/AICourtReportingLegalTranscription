...
/**
 * Gap/CFS Feature Page: `deliveries` lacks `/optimize-delivery-routing`.
 * Slug: deliveries-lacks-optimize-delivery-routing
 * Backend: /api/gap-deliveries-lacks-optimize-delivery-routing
 */
export default function DeliveriesLacksOptimizeDeliveryRoutingPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/gap-deliveries-lacks-optimize-delivery-routing', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>`deliveries` lacks `/optimize-delivery-routing`.</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>`deliveries` lacks `/optimize-delivery-routing`.</p>
