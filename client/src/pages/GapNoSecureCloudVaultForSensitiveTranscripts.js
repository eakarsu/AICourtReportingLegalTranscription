...
/**
 * Gap/CFS Feature Page: No secure cloud vault for sensitive transcripts.
 * Slug: no-secure-cloud-vault-for-sensitive-transcripts
 * Backend: /api/gap-no-secure-cloud-vault-for-sensitive-transcripts
 */
export default function NoSecureCloudVaultForSensitiveTranscriptsPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-secure-cloud-vault-for-sensitive-transcripts', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>No secure cloud vault for sensitive transcripts.</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>No secure cloud vault for sensitive transcripts.</p>
