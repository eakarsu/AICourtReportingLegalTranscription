...
/**
 * Gap/CFS Feature Page: No webhooks or notification system.
 * Slug: no-webhooks-or-notification-system
 * Backend: /api/gap-no-webhooks-or-notification-system
 */
export default function NoWebhooksOrNotificationSystemPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-webhooks-or-notification-system', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>No webhooks or notification system.</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>No webhooks or notification system.</p>
