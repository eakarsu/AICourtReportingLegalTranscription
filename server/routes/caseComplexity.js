/**
 * Custom Feature: case-complexity
 * Predictive case complexity scoring
 *
 * POST /api/ai/case-complexity
 * Auth required. Generated as part of Custom Feature Suggestions scaffold (batch_02).
 * Integration credentials: process.env.FEATURE_CASE_COMPLEXITY_KEY
...
const FEATURE_KEY = process.env.FEATURE_CASE_COMPLEXITY_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: Predictive case complexity scoring.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn(`${baseUrl}/chat/completions`, {
    ...
    headers: {
      ...
      'X-Title': 'AICourtReportingLegalTranscription - case-complexity',
    },
    body: JSON.stringify({
...
router.post('/case-complexity', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    if (!payload || Object.keys(payload).length === 0) {
      return res.status(400).json({ error: 'Request body is required' });
    }
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_CASE_COMPLEXITY_KEY');
    const ai = await callLLM({ feature: 'case-complexity', goal: 'Predictive case complexity scoring', input: payload });
    try {
      if (pool && pool.query) {
        await pool.query(
          ...
          ['case-complexity', JSON.stringify(payload), JSON.stringify(ai.result), ai.model, req.user?.id || null]
        );
      }
    } catch (persistErr) {
      console.warn('[case-complexity] persistence skipped:', persistErr.message);
    }
    return res.json({
      ok: true,
      feature: 'case-complexity',
      endpoint: '/api/ai/case-complexity',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
    ...
    console.error('[case-complexity] error:', err.message);
    return res.status(err.statusCode || 500).json({ error: err.message || 'Internal error' });
  }
});

router.get('/case-complexity/health', (req, res) => {
  res.json({
    feature: 'case-complexity',
    endpoint: '/api/ai/case-complexity',
